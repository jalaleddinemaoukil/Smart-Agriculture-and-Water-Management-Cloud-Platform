...
export default function SensorCard({ sensor }) {
  ...
  const getHealthStatus = () => {
    ...
    return { status: 'good', color: 'green', text: 'Optimal' };
  };
    ...
    >
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-start gap-3">
         <div>
            <h3 className="text-lg font-bold text-gray-800">{sensor.name || sensor.sensorId || 'Sensor'}</h3>
            <p className="text-sm text-gray-600">{sensor.location || 'Unknown Zone'}</p>
          </div>
        </div>

        <span className={`${badgeColors[health.color]} text-white px-2 py-1 rounded-full text-xs font-semibold`}>{health.text}</span>
      </div>

      <div className="grid grid-cols-3 gap-3 mb-3 text-center">
        <div>
          <p className="text-xs text-gray-600 mb-1 flex items-center justify-center gap-2"><Thermometer className="w-4 h-4"/> Temp</p>
          <p className="text-xl font-bold text-gray-800">{typeof sensor.temperature === 'number' ? `${sensor.temperature.toFixed(1)}°` : '—'}</p>
        </div>
        <div>
          <p className="text-xs text-gray-600 mb-1 flex items-center justify-center gap-2"><Droplet className="w-4 h-4"/> Humid</p>
          <p className="text-xl font-bold text-gray-800">{typeof sensor.humidity === 'number' ? `${sensor.humidity.toFixed(1)}%` : '—'}</p>
        </div>
        <div>
          ...
          <p className="text-xl font-bold text-gray-800">{typeof sensor.soilMoisture === 'number' ? `${sensor.soilMoisture.toFixed(1)}%` : '—'}</p>
        </div>
      </div>
